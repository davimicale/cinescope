...
export default function FilmeCard({
  ...
  return (
    <article
      className={`flex flex-col w-full ${className} max-w-full sm:max-w-[200px]`}
    >
      <Link href={`/filme/${id}`} className="relative w-full h-[300px] rounded-md overflow-hidden">
        <img
          src={imagem}
          alt={`Capa de ${titulo}`}
          title={titulo}
      ...
      <Link href={`/filme/${id}`} className="text-sm mt-2 font-medium line-clamp-2" title={titulo}>
        {titulo}
      </Link>
      ...
      <div className="flex justify-between mt-2 gap-1">
        ...
        <Star
          size={20}
          className={`cursor-pointer transition ${
            favorito ? 'text-yellow-400' : 'text-gray-400 hover:text-yellow-500'
          }`}
          onClick={onFavoritar}
